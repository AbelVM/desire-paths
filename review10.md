...
| **P1** | SAB atomic shared footprint (the *only* dynamics-safe parallelism) | med (determinism) | L | Very High (throughput) — unlocks multi-worker agent stage without breaking dynamics; costs cross-pair non-determinism. Footprints are now already a `Uint32Array(V)` (SAB-ready). | ✅ done (flag-guarded, §9) |
...
## 9. Fifth implementation pass — P1 SAB atomic shared-footprint parallelism (all green: 357 tests, 6 skipped)

The last open item from the review. Implemented the dynamics-safe agent parallelism; the footprint accumulator is now **persistent across waves** (see §10) and the parallelism flag defaults ON (the only hard gate is `crossOriginIsolated`, a browser requirement for `SharedArrayBuffer`, not a legacy-compat concern — the app is pre-release).

- **SAB atomic footprint accumulator (`agentTasks.js`).** `computeAgentBatch` now accepts `options.footprintBuffer` — a typed array (length >= graph.V) the caller owns and reuses. When it is SAB-backed (page cross-origin isolated) the per-cell wear write uses `Atomics.add(abmFootprints, gi, 1)`; otherwise plain `++` (single-worker). The scorer's footprint *reads* stay plain typed-array subscripts (correct for a shared int, no torn reads). Falls back to a private `Uint32Array(V)` when no buffer is supplied (legacy single-call semantics). Verified byte-identical to the local path for one batch (`parallelAgentBatches.test.js`).
- **Multi-worker sharding (`spatialWorker.js`).** `runAgentBatches` has a default-ON `parallelAgentBatches` flag (module-level `setParallelAgentBatches`/`getParallelAgentBatches`, mirrored on `window` like `setMaxAgentWorkers`, overridable via `options.parallelAgentBatches`). When enabled **and** `Worker` exists **and** the page is `crossOriginIsolated` (SAB availability) **and** a valid gradient graph exists **and** the plan has >1 entry that actually parallelizes, it shards the plan across up to `MAX_AGENT_WORKERS` `agent-batch` workers. All shards share the caller-provided `SharedArrayBuffer(V*4)` footprint (reused across waves; posted *without* a transfer list so it is shared zero-copy — the existing S1-SAB path already keeps the friction/affordance buffers SAB-backed + untransferred under `crossOriginIsolated`). Each worker builds its own identical graph (same `frictionEntries`/`viewHexes`/`r1Adjacency` ⇒ same `cellToIdx` ⇒ consistent SAB indices) and accumulates wear via `Atomics.add`.
- **Cross-worker merge (`mergeAgentResults`, exported).** `pathDesire` and `perTargetContribs` have no cross-shard interaction (each shard owns its pairs), so they are merged by summation on the main thread; `processed`/`total` are summed for progress. `runParallelAgentBatches` dispatches the shards via `Promise.all(runWorker('agent-batch', …))` and merges the normalized results. Any failure falls back to the single-worker path.
- **Plumbing.** `compute.js` creates/reuses `state._footprintBuffer` (sized to the current gradient graph `V`; SAB when `crossOriginIsolated`, else a plain `Uint32Array`) and passes it as `footprintBuffer` into the `runAgentBatches` options, so it persists across waves and is shared within a wave. `shardPlanForAgents` (exported, wraps `splitIntoChunks`) is the sharding helper.
- **New tests** `tests/parallelAgentBatches.test.js` (6 cases): SAB path byte-identical to local for one batch; shared SAB accumulates exactly 2× across two "worker" runs (affordanceWeight=0 so paths are wear-independent); `mergeAgentResults` sums pathDesire/perTargetContribs; `shardPlanForAgents` chunking; `runAgentBatches` with the flag is a safe no-op in Node (no `Worker` ⇒ single-worker fallback, output equals the unflagged run); and a **persistent `footprintBuffer` is reused across two `runAgentBatches` calls** (wave n+1 sees 0…n's wear). The true multi-worker dispatch only executes in a cross-origin-isolated browser (where `SharedArrayBuffer` + `Worker` coexist), which the Node test environment cannot spawn — the sharding + atomic-accumulator + merge + wave-persistence logic is fully covered without real workers.

**All review10 items are now implemented.** The only accepted behavioral change is that, when the multi-worker path engages (cross-origin-isolated browser + parallelizable plan), cross-pair ordering becomes non-deterministic (statistically-equivalent emergent output, same tolerance the existing T>0 tests already accept). The single-worker path remains the fallback and is byte-deterministic within a wave; footprints now persist across waves by default (the intended ABM dynamics for wave-based simulation).

## 10. Footprint reset wiring (wave-persistence follow-up)

The persistent `state._footprintBuffer` (§9) is reset to `undefined` at the two UI entry points that should start a fresh simulation:
- **"Reveal desire lines"** (`handleComputeClick`, `ui.js`): `map._footprintBuffer = undefined` immediately before `computeDesirePaths()`, so each reveal runs from zero accumulated wear. Waves *within* that reveal still accumulate into the freshly-created buffer.
- **"Clear map"** (`resetSimulationState`, `ui.js`): `map._footprintBuffer = undefined` alongside the other simulation-state resets, so a cleared map has no stale wear.

`compute.js` recreates the buffer (sized to the current graph `V`, SAB when isolated) on the next run whenever it is `undefined` or too small.
