  ...
  2. **Blur crosses tier boundaries (#3) — CORRECTED.** The original review
     ...
     **That is incorrect.** The blur is applied with a gaussian weight per
     ...
     skip the cell (they skip only exact `FRICTION_COSTS.PAVEMENT`/IMPASSABLE), so
      ...
      — ✅ Accepted (by design): the `pavement`→`light_park` tier crossing is
      intentional (a cell next to a building *should* read as rougher ground, and
      the affordance reclassification is the desired "less walkable" signal). The
      suggested midpoint cap was rejected. Instead, the blur now **accumulates**
      the gaussian weight from *every* impassable source within `radius` (not just
      the nearest), so concave corners and alcoves — bordered by several barriers
      — receive the SUM of their contributions and are penalized more than a cell
      next to a single barrier. Covered by the "accumulates blur from multiple
      adjacent impassable sources (concave corners)" regression test.
 ...
 2. **🔴 `getBestNextStep` fallback returns a bearing, not a cell (#2).**
  ...
  3. **🟠 Blur tier flip (#3) — CORRECTED.** See §2.2. The original claim that
     ...
     gaussian-weighted blur yields `light_park` (2.82), not `heavy_grass`. The
      ...
      — ✅ Accepted (by design): the tier crossing is intentional; the blur now
      **accumulates** the gaussian weight from every impassable source within
      `radius` (not just the nearest), so concave corners/alcoves are penalized
      more. The suggested midpoint cap was rejected. Covered by the "accumulates
      blur from multiple adjacent impassable sources (concave corners)" regression
      test.
...
3. **Harden #4** (retry/fallback on mapping-graph/visibility/merge/gradient
   ...
   4. **Blur tier crossing #3 (corrected) — Accepted (by design).** Blur crosses
      `pavement`→`light_park` (not `heavy_grass` as originally claimed). The
      suggested midpoint cap was rejected; instead the blur now **accumulates**
      the gaussian weight from *every* impassable source within `radius`, so
      concave corners/alcoves (bordered by several barriers) are penalized more
      than a cell next to a single barrier — the desired "rougher ground" signal.
      Covered by the "accumulates blur from multiple adjacent impassable sources
      (concave corners)" regression test.
...
## 12. Pending items summary

| # | Item | Status | Recommendation | Advantages of implementing | Trade-offs / Risks |
|---|------|--------|----------------|----------------------------|-------------------|
| 1 | `precomputeOriginDestDistances` O(O·D) startup stall | 🟡 Open | Defer | Removes one-time sim-start stall; lazy/memoized compute only for actually queried pairs | Adds memoization complexity; first query of a new pair still pays Dijkstra cost |
| 2 | `renderInterfacePins` AOI recompute on mousemove | 🟡 Open | Defer | Eliminates per-frame polygon reprojection during drag; rAF throttle reduces main-thread jank | Slight input latency from rAF throttle; negligible for small N |
| 3 | `updateLayers` full flat/flow rebuild on version bump | 🟡 Open | Defer | Avoids redundant layer rebuilds when edits touch disjoint cell subsets; scales better with frequent edits | Dirty-region tracking adds bookkeeping; full rebuild is simpler and already correct |
| 4 | Two sources of truth for sim params | ✅ Done | — | `state.simulationParams` is now the single live module object via getter/setter in `main.js`; `updateSimulationParams` writes through to `SIMULATION_PARAMS` so the two can never diverge | Low risk; migration completed |
| 5 | State ownership centralization (SimulationStore) | 🟡 Open | Defer | Localizes invalidation rules in one place; removes ad-hoc mutations from 6 modules | Moderate refactor across `grid.js`, `compute.js`, `map.js`, `ui.js`, `surfaceEdition.js`; Proxy removal already done (#8) |
| 6 | Worker RPC typed wrapper / Comlink | 🟡 Open | Won't do | Removes `runLocally`/`runWorker` branching duplication; type-safe message contract | Adds dependency (Comlink) or thin abstraction; current hand-rolled protocol is functional |
| 7 | Surface Edition coverage bitmap / dirty-set | 🟡 Open | Defer | Cheaper re-apply for large polygons; O(1) union instead of O(total edited cells) per feature | More complex edit storage; current full cell list is simple and correct |
| 8 | `updateAffordance`/`decayAffordance` tier check | 🟠 Open | Won't do (accepted by design) | Blurred pavement cells stop being worn as light park; restores "permanent infrastructure" intent for near-building pavement | Requires tier-range skip logic instead of exact-match; may under-wear genuinely rougher ground |
| 9 | `isAccessible` bounded retry loop | ✅ Done | — | Handles slow tile loads more robustly than single 150 ms retry; `maxAttempts = 3` in `main.js:645` | Slightly longer wait on genuinely missing tiles; bounded loop prevents hangs |
| 10 | `computeDesirePaths` rejection handling | ✅ Done | — | Guarantees `isComputing` resets on gradient failure; prevents stuck UI busy state via `try/catch/finally` in `ui.js:1513–1525` | Low risk; `finally` block ensures busy-state reset |
| 11 | Debug globals gated behind `import.meta.env.DEV` | ✅ Done | — | Removes `window.__map`, `window.__td`, `window.__dp_*` from production bundles; all three sites now gated in `main.js`, `spatialWorker.js`, `surfaceEdition.js` | Zero runtime cost; purely cleanup |
| 12 | Inline magic numbers centralized | 🟡 Open | Defer | Single source of truth for tunables; easier calibration | Low risk; mostly mechanical extraction |
| 13 | `getBestNextStep` candidate-builder refactor | 🟡 Open | Won't do | The `_kn*` module-level reusable buffers are an intentional hot-path pattern (avoids per-call allocation on millions of invocations); a builder object would add indirection without measurable gain | Functional correctness unchanged; refactor risk outweighs readability gain for a performance-critical path |
| 14 | `useGradient` naming clarification | 🟡 Open | Defer | Removes overloaded meaning ("gradient object present" vs "gradient mode") | Low risk; rename + update all call sites |
| 15 | `getScore`/`getPathScore` direct Float32Array read | 🟡 Open | Won't do | `cellFrictionMap` (Map) is the canonical friction store; `_frictionObj` is a Map view, not a Float32Array, and the dual get/bracket dispatch is already branch-predicted by V8 | Requires `_frictionObj` canonical array view guarantee; marginal gain if Map is already hot |
